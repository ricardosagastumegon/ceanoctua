...
import { terrestreTotal } from './api';
import type { AttTerrestre, AttTerrestreInsert } from './api';

type TipoServicio = 'Privada' | 'Colectiva' | 'Otro' | '';

export function TerrestreForm({ open, viajeId, editing, submitting, onClose, onSubmit }: {
  open: boolean; viajeId: string; editing: AttTerrestre | null;
  submitting: boolean; onClose: () => void;
  onSubmit: (values: AttTerrestreInsert) => void | Promise<void>;
}) {
  ...
  useEffect(() => {
    ...
    if (editing) {
      ...
      setPrestador(''); setCiudad(''); setTipoVeh('');
      setTipoServicio('Privada'); setTipoServicioOtro('');
      setReservaNombre(''); setPersonas(''); setConfirmacion('');
  ...
  const totalPreview = useMemo(
    () => terrestreTotal({ tarifa: Number(tarifa || 0), personas: Number(personas || 0), monto_extras: Number(montoExtras || 0) } as AttTerrestre),
  ...
  return (
    <Modal open={open} onClose={onClose} title={`${meta.icon} ${editing ? 'Editar' : 'Nuevo'} Traslado Terrestre`} size="lg">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="Prestador *" value={prestador} onChange={(e) => setPrestador(e.target.value)} placeholder="City Transfers" autoFocus />
          <TextInput label="Ciudad" value={ciudad} onChange={(e) => setCiudad(e.target.value)} />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <TextInput label="Tipo de vehículo" value={tipoVeh} onChange={(e) => setTipoVeh(e.target.value)} placeholder="Van, sedán ejecutivo" />
          <div>
        ...
        <TextInput label="Inclusiones" value={inclusiones} onChange={(e) => setInclusiones(e.target.value)} placeholder="Agua, wifi…" />
        <div className="grid grid-cols-3 gap-3">
        ...
        <div style={{ background: meta.grad }} className="flex items-center justify-between rounded-lg px-4 py-3 text-white">
          <div>
            ...
            <div className="text-[10px] text-white/50">Tarifa × personas + extras</div>
          </div>
        ...
        <div className="flex justify-end gap-2 pt-2">
          ...
          <button type="submit" disabled={submitting} style={{ backgroundColor: meta.solid }} className="rounded-md px-4 py-2 text-sm font-semibold text-white disabled:opacity-50">
            {submitting ? 'Guardando…' : '💾 Guardar Traslado Terrestre'}
          </button>
        </div>
      </form>
    </Modal>
