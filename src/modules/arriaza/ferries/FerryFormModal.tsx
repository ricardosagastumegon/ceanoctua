...
import { attFerriesByViajeKey, attFerriesKey } from './hooks';
import {
  ferriesFullApi,
  subirConfirmacionFerry,
  totalTarifaExtras,
  type AttFerryInsert,
} from './full-api';
...
/** Del documento: qué se transporta en el ferry. */
export const SERVICIO_PARA = ['Personas', 'Vehículos', 'Persona & Vehículo'] as const;
export type ServicioPara = (typeof SERVICIO_PARA)[number];
...
/** Formulario de servicio ferry. */
export function FerryFormModal({ open, viajeId, ferryId, onClose }: Props) {
  const qc = useQueryClient();
  const toast = useToast();
  const cargado = useQuery({
    queryKey: ['att_ferry_full', ferryId],
    queryFn: () => ferriesFullApi.load(ferryId as string),
    enabled: open && !!ferryId,
  });
  const save = useMutation({
    mutationFn: (vars: Parameters<typeof ferriesFullApi.save>[0]) => ferriesFullApi.save(vars),
    onSuccess: (id) => {
  ...
  useEffect(() => {
    ...
    setServicioPara((a?.servicio_para as ServicioPara | null) ?? 'Personas');
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    const cabecera: AttFerryInsert = {
      ...
      servicio_para: servicioPara || null,
    ...
    try {
      await save.mutateAsync({ id: ferryId, cabecera });
      toast.success(ferryId ? 'Servicio ferry actualizado.' : 'Servicio ferry agregado.');
      onClose();
    } catch (err) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={`${META.icon} ${ferryId ? 'Editar' : 'Nuevo'} Servicio Ferry`}
      size="xl"
    >
      {cargado.isLoading ? (
        <p className="text-sm text-dark-3">Cargando ferry…</p>
      ) : (
        <form onSubmit={handleSubmit} className="space-y-4">
          <Bloque titulo="El prestador">
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <TextInput label="Prestador de servicios *" value={prestador} onChange={(e) => setPrestador(e.target.value)} placeholder="Ej: Isla Ferries" autoFocus />
              <TextInput label="Ciudad" value={ciudad} onChange={(e) => setCiudad(e.target.value)} />
            </div>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <TextInput label="Dirección" value={direccion} onChange={(e) => setDireccion(e.target.value)} placeholder="Dirección / terminal" />
              <TextInput label="Teléfono" value={telefono} onChange={(e) => setTelefono(e.target.value)} />
            </div>
          </Bloque>

          <Bloque titulo="La embarcación">
            <TextInput label="Tipo de embarcación" value={tipoEmbarcacion} onChange={(e) => setTipoEmbarcacion(e.target.value)} placeholder="Ej: Ferry de pasajeros" />
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
              <Select label="Servicio para" value={servicioPara} onChange={(e) => setServicioPara(e.target.value as ServicioPara)}>
                {SERVICIO_PARA.map((s) => <option key={s} value={s}>{s}</option>)}
              </Select>
          ...
          <Bloque titulo="Información de precio">
            <TextArea label="Inclusiones" value={inclusiones} onChange={(e) => setInclusiones(e.target.value)} rows={2} placeholder="Ej: Asientos preferentes…" />
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
          ...
          <Bloque titulo="Confirmación">
            {ferryId ? (
              <ConfirmacionBoton id={ferryId} actual={cargado.data?.confirmacion_path} />
            ) : (
              <p className="text-xs italic text-dark-3">
                Guarda el ferry primero — el archivo se guarda bajo su número.
              </p>
            )}
          </Bloque>
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {save.isPending ? 'Guardando…' : '💾 Guardar Servicio Ferry'}
            </button>
          </div>
        </form>
      )}
...
function ConfirmacionBoton({ id, actual }: { id: string; actual?: string | null }) {
  ...
  async function handleFile(file: File | null) {
    ...
    setSubiendo(true);
    try {
      await subirConfirmacionFerry(id, file);
      await qc.invalidateQueries({ queryKey: ['att_ferry_full', id] });
